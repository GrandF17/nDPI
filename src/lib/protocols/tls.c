...
void processCertificateElements(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    for (i = p_offset; i < certificate_len - 2; i++) {
        ...
        */
        if ((packet->payload[i] == 0x55) && (packet->payload[i + 1] == 0x04) && (packet->payload[i + 2] == 0x03)) {
            ...
            if (i + 3 < certificate_len &&
                ...
                if ((offset + 1) < packet->payload_packet_len) {
                    ...
                    if ((offset + len) < packet->payload_packet_len) {
                        ...
                        } else if ((time_sec > flow->protos.tls_quic.notBefore) && (time_sec > (flow->protos.tls_quic.notAfter - (ndpi_struct->cfg.tls_certificate_expire_in_x_days * 86400)))) {
                            char str[96], b[32], e[32];
                            struct tm result;
                            time_t theTime;
            ...
            if (i < packet->payload_packet_len && packet->payload[i] == 0x04)
                i++;
            else
                i += 4; /* 4 bytes, with the last byte set to 04 */
                    ...
                    if (i < packet->payload_packet_len) {
                        ...
                        while (i < packet->payload_packet_len) {
                            ...
                            if ((general_name_type == 0x81)    /* rfc822Name */
                                ...
                                if ((i < (packet->payload_packet_len - 1)) && ((i + packet->payload[i + 1] + 2) < packet->payload_packet_len)) {
                                    ...
                                    if ((len == 0 /* Looks something went wrong */) || ((i + len) > packet->payload_packet_len))
                                        break;
    ...
    if (flow->protos.tls_quic.subjectDN && flow->protos.tls_quic.issuerDN && (!strcmp(flow->protos.tls_quic.subjectDN, flow->protos.tls_quic.issuerDN))) {
...
int processCertificate(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    while (certificates_offset < certificates_length) {
        ...
        /* Invalid lenght */
        if ((certificate_len == 0) || (packet->payload[certificates_offset] != 0x0) || ((certificates_offset + certificate_len) > (4 + certificates_length + (is_dtls ? 8 : 0)))) {
#ifdef DEBUG_TLS
    ...
    if ((ndpi_struct->num_tls_blocks_to_follow != 0) && (flow->l4.tcp.tls.num_tls_blocks >= ndpi_struct->num_tls_blocks_to_follow)) {
...
static int ndpi_search_tls_tcp(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    while (!something_went_wrong) {
        ...
        if ((len > 9) && (content_type != 0x17 /* Application Data */)) {
    ...
    if (something_went_wrong || ((ndpi_struct->num_tls_blocks_to_follow > 0) && (flow->l4.tcp.tls.num_tls_blocks == ndpi_struct->num_tls_blocks_to_follow)) || ((ndpi_struct->num_tls_blocks_to_follow == 0) && (/* Common path: found handshake on both directions */
                                                                                                                                                                                                                 ...
                                                                                                                                                                                                                 (flow->protos.tls_quic.client_hello_processed && flow->l4.tcp.tls.app_data_seen[!flow->protos.tls_quic.ch_direction] == 1) || (flow->protos.tls_quic.server_hello_processed && flow->l4.tcp.tls.app_data_seen[flow->protos.tls_quic.ch_direction] == 1)))) {
#ifdef DEBUG_TLS_BLOCKS
...
static void ndpi_compute_ja4(struct ndpi_detection_module_struct *ndpi_struct,
    ...
    */
    ...
    for (i = 0; i < ja->client.num_supported_versions; i++) {
        if ((!is_grease_version(ja->client.supported_versions[i])) && (tls_handshake_version < ja->client.supported_versions[i]))
    ...
    for (i = 0; i < ja->client.num_ciphers; i++) {
        ...
        if ((rc > 0) && (tmp_str_len + rc < JA_STR_LEN))
            tmp_str_len += rc;
        else
            break;
    }
...
#ifndef JA4R_DECIMAL
    ja_str[ja_str_len] = 0;
    i = snprintf(&ja4_r[ja4_r_len], sizeof(ja4_r) - ja4_r_len, "%s", ja_str);
    if (i > 0) ja4_r_len += i;

    tmp_str[tmp_str_len] = 0;
    i = snprintf(&ja4_r[ja4_r_len], sizeof(ja4_r) - ja4_r_len, "%s_", tmp_str);
    if (i > 0) ja4_r_len += i;
#endif
    ...
    for (i = 0, num_extn = 0; i < ja->client.num_tls_extensions; i++) {
        if ((ja->client.tls_extension[i] > 0) && (ja->client.tls_extension[i] != 0x10 /* ALPN extension */)) {
            ...
            if ((rc > 0) && (ja4_r_len + rc < JA_STR_LEN))
                ja4_r_len += rc;
            else
                break;
#endif
        ...
        if (handshake_type == 0x02 /* Server Hello */) {
            ...
            */
            ...
            if (flow->protos.tls_quic.ssl_version < 0x0303) /* < TLSv1.2 */ {
                ...
                snprintf(str, sizeof(str), "%s", ndpi_ssl_version2str(buf, sizeof(buf), flow->protos.tls_quic.ssl_version, &unknown_tls_version));
            ...
            if (offset < total_len) {
                ...
                if (offset + 1 < total_len) {
                    ...
                    if ((extensions_len + offset) <= total_len) {
                            ...
                            if (extension_id == 0 /* server name */) {
                                ...
                                if ((offset + extension_offset + 4) < packet->payload_packet_len) {
                                    ...
                                    if ((offset + extension_offset + 5 + len) <= packet->payload_packet_len) {
                                        ...
                                        if ((flow->protos.tls_quic.subprotocol_detected == 0) && (check_sni_is_numeric_ip(sni) == 1)) {
                                        ...
                                        if (ndpi_check_dga_name(ndpi_struct, flow, sni, 1, 0)) {
                                            ...
                                            if ((sni_len >= 4)
                                                ...
                                                && ((strcmp(&sni[sni_len - 4], ".com") == 0) || (strcmp(&sni[sni_len - 4], ".net") == 0)) && (strncmp(sni, "www.", 4) == 0)) /* Not starting with www.... */
...
#endif
                                        if ((s_group == 0) || (packet->payload[s_offset + i] != packet->payload[s_offset + i + 1]) || ((packet->payload[s_offset + i] & 0xF) != 0xA)) {
                                            /* Skip GREASE */
                                    ...
                                    if (last_signature == signature_algo) {
                                        ...
                                        for (j = 0; j < tot_signature_algorithms_len; j += 2) {
                                            if (j != i && s_offset + (int)j + 2 < packet->payload_packet_len) {
                                                ...
                                                if ((signature_algo == j_signature_algo) && (i < j) /* Don't skip both of them */) {
#ifdef DEBUG_HEURISTIC
                                ...
                                if ((flow->protos.tls_quic.browser_heuristics.is_chrome_tls == 0) && duplicate_found)
                            ...
                            if (ndpi_struct->cfg.tls_ja3c_fingerprint_enabled) {
                                ...
                                for (i = 0; i < ja.client.num_ciphers; i++) {
                                    rc = ndpi_snprintf(&ja_str[ja_str_len], JA_STR_LEN - ja_str_len, "%s%u",
                                                       (i > 0) ? "-" : "", ja.client.cipher[i]);
                                    if ((rc > 0) && (ja_str_len + rc < JA_STR_LEN))
                                        ja_str_len += rc;
                                    else
                                        break;
                                }
                        ...
                        if ((flow->protos.tls_quic.ssl_version >= 0x0303) /* >= TLSv1.2 */
                            && !flow->protos.tls_quic.webrtc && (flow->protos.tls_quic.advertised_alpns == NULL) /* No ALPN */) {
                        ...
                        if (flow->host_server_name[0] == '\0' && (flow->protos.tls_quic.ssl_version >= 0x0302)     /* TLSv1.1 */
                            && !flow->protos.tls_quic.webrtc && (flow->protos.tls_quic.encrypted_sni.esni == NULL) /* No ESNI */
                        ) {
